...
| B — Structure | Hand off live-code structure to `global-refactor` (language skills) |
...
external consumers. Use `global-code-investigation` when reachability is
uncertain.
...
- [ ] 3. Phase B — Structure via global-refactor (live code only)
...
4. For Phase B, follow `.cursor/skills/global-refactor/SKILL.md` (dispatches to
...
anything remains (often: `global-refactor` hygiene phase 3, or stop).

...
description: >-
  Rewrite English so an agent or downstream system cannot misread it —
  tool descriptions, error messages, inter-agent instructions, system
  prompts, status reports — or when text is dense, hedged, or ambiguous.
  Triggers: disambiguate, STE100, Simplified Technical English, plain-language
  rewrite, controlled-language rewrite, rewrite so an agent cannot misread
  this. Not for creative or marketing copy. Companion to global-action-first
  (reply shape); this skill owns Strict / STE rewrite jobs.
version: 0.4.0
---

# Simplified Technical English (ASD-STE100)

Portable `global-*` skill. Copied from
...
## Wire with `global-action-first`

| Job | Skill |
| --- | --- |
| Normal chat / coding handoff | `global-action-first` — lead with the result; use STE-flavored **sentence** habits inside that shape |
| Explicit rewrite / disambiguate / "apply STE" on a pasted string | **This skill** — Strict or STE-flavored rewrite; default output is the rewritten text alone |

Do not replace an action-first reply with a bare STE paste unless the user asked only for a rewrite of supplied text. Do not run Strict mode on every conversational answer.

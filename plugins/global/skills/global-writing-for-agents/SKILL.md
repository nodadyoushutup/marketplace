...
description: >-
  Write documents agents consume: skills, AGENTS.md / CLAUDE.md, and docs
  reached by pointers. Use when creating or editing skills, skill descriptions,
  or agent guidance files. Pair with global-asd-ste100 for unambiguous wording
  and global-policy-evolution when promoting durable policy.
---

# Writing for agents

Portable `global-*` skill. Adapted from mattpocock/skills `writing-for-agents`
(MIT); see `ORIGIN.md`.

When the document is a skill, also read [`SKILL-MECHANICS.md`](SKILL-MECHANICS.md).
Prefer STE-flavored clarity (`global-asd-ste100`) in skill bodies and triggers.
When promoting durable rules/skills into this repo, follow
`global-policy-evolution` and `global-skill-intake` for supply-chain review.

Reference for writing any document an agent consumes: a skill, an `AGENTS.md` /
`CLAUDE.md`, a doc reached by a pointer. The packaging differs; the writing
does not: the same levers make each one predictable, since the agent takes the
same _process_ every run rather than producing the same output.
